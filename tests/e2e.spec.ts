...
test('Sign in', async ({ page }) => {
  ...
  await page.getByLabel('Email').fill('sbs-06@mail.ru');
  await page.getByLabel('Пароль').fill('12345Qwerty');
...
test('Sign up', async ({ page }) => {
  ...
  await page.getByLabel('Имя').fill('Tester');
  await page.getByLabel('Email').fill(`${Date.now()}@test.ru`);
  await page.getByLabel('Пароль:', {exact: true}).fill('12345Qwerty1');
  await page.getByLabel('Повторите пароль:', {exact: true}).fill('12345Qwerty1');
  await page.getByTestId('signInOrSignUpButton').click();
